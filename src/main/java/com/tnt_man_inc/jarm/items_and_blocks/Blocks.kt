 ...
 * @see registerBlock for how it registers
 */
object Blocks {
    ...
    val NISGD_BLOCK = registerBlock("netherite_infused_strong_guilded_diamond_block", Main.JARM_GROUP, Block(FabricBlockSettings.of(Material.METAL).hardness(8.0f)))
    val RUBY_LOG: Block = registerBlock("ruby_log", Main.JARM_GROUP, Block(FabricBlockSettings.of(Material.NETHER_WOOD).hardness(2f)))
    val RUBY_LEAVES: LeavesBlock = registerBlock("ruby_leaves", Main.JARM_GROUP, LeavesBlock(AbstractBlock.Settings.of(Material.LEAVES).strength(0.2F).ticksRandomly().sounds(BlockSoundGroup.GRASS).nonOpaque().allowsSpawning(BlocksAccessor::callCanSpawnOnLeaves).suffocates(BlocksAccessor::callNever).blockVision(BlocksAccessor::callNever))
    )
